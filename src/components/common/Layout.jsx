...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const pollIncomingCalls = async () => {
      try {
        ...
        const { data: rooms } = await api.post("/communication/summary", {
          roomIds,
        });
        if (!isMounted) return;
        ...
        if (user.role === "Patient") {
          ...
          apptList.forEach((appt) => {
            const feesToCheck =
              appt.feeHistory && appt.feeHistory.length > 0
                ? appt.feeHistory
                : appt.feeAmount > 0
                  ? [
                      {
  ...
  useEffect(() => {
    ...
    const checkPrescriptions = async () => {
      try {
        ...
        const currentWindow = isMorning
          ? "Morning (8am - 10am)"
          : isAfternoon
            ? "Afternoon (2pm - 4pm)"
            : "Night (9pm - 11pm)";
        const dosageKey = isMorning
          ? "morning"
          : isAfternoon
            ? "afternoon"
            : "night";
  ...
  const handleAnswer = (call) => {
    ...
    const contactId =
      user.role === "Doctor"
        ? call.appointment.patient?._id
        : call.appointment.doctor?._id;
  ...
  const handleDecline = async (call) => {
    ...
    setIncomingCall(null);
    try {
      await api.post(`/communication/${call.roomId}/signal`, {
        clearSignal: true,
      });
    } catch (err) {
  ...
  return (
    <div
      className={`app-layout${isConsultation ? " consultation-layout-active" : ""}${isChatting ? " chatting-active" : ""}`}
    >
      <style>{`
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            >
              {(
                (user?.role === "Doctor"
                  ? incomingCall.appointment?.patient?.name
                  : incomingCall.appointment?.doctor?.name) || "U"
              )
                .replace(/^Dr\.\s*/i, "")
                .charAt(0)
                .toUpperCase()}
            ...
            <div>
              ...
              >
                {user?.role === "Doctor"
                  ? incomingCall.appointment?.patient?.name || "Patient"
                  : `Dr. ${(incomingCall.appointment?.doctor?.name || "Doctor").replace(/^Dr\.\s*/i, "")}`}
              </p>
            </div>
          </div>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              ...
              >
                With Dr. {upcomingAppointment.doctor?.name || "Doctor"} at{" "}
          ...
          <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
            {upcomingAppointment.feeAmount > 0 &&
            upcomingAppointment.feeStatus !== "Paid" ? (
              <button
              ...
              <button
                onClick={() =>
                  navigate("/consultation", {
                    state: { autoSelectAppointmentId: upcomingAppointment._id },
                  })
                }
                style={{
                  background: "#10b981",
      ...
      <main
        ...
        style={
          isChatting
            ? { padding: "16px", maxWidth: "100%", flex: 1 }
            : undefined
        }
      >
        {children}
